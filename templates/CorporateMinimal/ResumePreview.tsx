...
const CustomListItem: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <li className="relative pl-5 mb-2 last:mb-0 before:content-[''] before:absolute before:left-0 before:top-[0.55em] before:w-1.5 before:h-1.5 before:bg-gray-600 before:rounded-full leading-relaxed">
      {children}
  </li>
);
...
const ResumePreview = forwardRef<HTMLDivElement, ResumePreviewProps>(({ resumeData }, ref) => {
  ...
  return (
    <div ref={ref} className="bg-white min-h-[297mm] w-full p-8 text-gray-800" style={{ fontFamily: 'Lato, sans-serif' }}>
        {/* Header */}
        ...
        <hr className="border-t border-gray-300 mb-8 -mx-8" />
        ...
        <div className="flex flex-row gap-8">
                ...
                {education && education.length > 0 && (
                    <section>
                        ...
                        <div className="space-y-6">
                            {education.map(edu => (
                                <div key={edu.id}>
                                    <div className="font-bold text-base uppercase text-gray-900">{edu.institution}</div>
                                    <div className="font-bold text-sm text-gray-700">{edu.degree}</div>
                                    <div className="text-sm text-gray-600 italic">{edu.year}</div>
                ...
                {webLinks && webLinks.length > 0 && (
                     <section>
                        ...
                        <div className="space-y-1 text-sm">
                            {webLinks.map(link => (
                                <div key={link.id}>
                                    <span className="text-gray-600">{link.name}://</span> <span className="font-medium text-gray-700">{link.url}</span>
                                </div>
                            ))}
                        </div>
                    </section>
                ...
                {coursework && coursework.length > 0 && (
                     <section>
                        ...
                        <div className="space-y-4">
                            {coursework.map(cw => (
                                <div key={cw.id}>
                                    <div className="font-bold text-sm uppercase mb-1 text-gray-800">{cw.category}</div>
                ...
                {skills && skills.length > 0 && (
                     <section>
                        ...
                        <div className="space-y-4">
                            {skills.map(skill => (
                                <div key={skill.id}>
                                    <div className="font-bold text-sm uppercase mb-1 text-gray-800">{skill.category}</div>
            ...
            <div className="flex-grow flex flex-col gap-8">
                    ...
                    <section>
                        ...
                        <div className="space-y-6">
                            {projects.map(proj => (
                                <div key={proj.id}>
                                    <div className="flex justify-between items-baseline mb-1">
                                        <div className="font-bold text-base uppercase text-gray-900">{proj.name}</div>
                                        <div className="text-sm text-gray-500 italic font-medium">{proj.date}</div>
                                    </div>
                                    <div className="text-sm text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: renderHTML(proj.description) }}></div>
                                </div>
                            ))}
                        </div>
                    </section>
                        ...
                        <ul className="text-sm text-gray-700">
                            {achievements.map(ach => (
                                <CustomListItem key={ach.id}>
                                    <span dangerouslySetInnerHTML={{ __html: ach.description }}></span>
                                </CustomListItem>
                            ))}
                        </ul>
                     </section>
                        ...
                        <ul className="text-sm text-gray-700">
                             {positions.map(pos => (
                                 <CustomListItem key={pos.id}>
                                     <span dangerouslySetInnerHTML={{ __html: renderHTML(pos.description) }}></span>
                                 </CustomListItem>
                             ))}
                        </ul>
                     </section>
                 ...
                 {activities && activities.map((act) => (
                    act.title.includes('EXTRA') && act.description && (
                        <section key={act.id}>
                                 ...
                                 {act.description.split('\n').map((line, i) => (
                                     line.trim() && (
                                        <CustomListItem key={i}>
                                            <span dangerouslySetInnerHTML={{ __html: line.replace(/<b>/g, '<strong>').replace(/<\/b>/g, '</strong>') }}></span>
                                        </CustomListItem>
                                     )
                                 ))}
                             </ul>
