...
type DynamicSectionKey = Exclude<keyof ResumeData, 'personalDetails' | 'activities' | 'summary' | 'skills'>;
...
const ResumeForm: React.FC<ResumeFormProps> = ({ resumeData, setResumeData, photoFileInputRef, logoFileInputRef }) => {
  ...
  // Special handler for the flattened skills section
  const handleSkillsChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
     const value = e.target.value;
     // Always maintain a single item in the array for this template
     setResumeData(prev => ({
         ...prev,
         skills: [{ id: 'skill-set-1', category: '', skills: value }]
     }));
  };
  ...
  function onImageLoad(e: React.SyntheticEvent<HTMLImageElement>) {
    ...
    const aspect = editingImageFor === 'logo' ? 1 : 1; 
  ...
  return (
    <div className="p-4 bg-white relative">
      ...
      >
        ...
        <Input label="Title/Degree" name="degree" value={resumeData.personalDetails.degree} onChange={handlePersonalChange} />
        ...
        <Input label="GitHub URL" name="github" value={resumeData.personalDetails.github || ''} onChange={handlePersonalChange} />
      </FormSection>
      ...
      >
        {resumeData.education.map((edu) => (
          <div key={edu.id} className="border-b border-gray-100 pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
            ...
            <Input label="Grade" name="grade" value={edu.grade} onChange={(e) => handleDynamicChange('education', edu.id, e)} />
            <div className="flex justify-end">
                <button onClick={() => removeDynamicItem('education', edu.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs font-semibold">Remove</button>
            </div>
          </div>
        ))}
        <button onClick={() => addDynamicItem('education', {id: crypto.randomUUID(), year: '', degree: '', institution: '', grade: ''})} className="mt-4 w-full py-2 border-2 border-dashed border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 hover:border-purple-300 transition-colors font-semibold text-sm">+ Add Education</button>
      </FormSection>
            ...
            <div key={intern.id} className="border-b border-gray-100 pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                ...
                <div className="flex items-center justify-between mt-3">
                    <button
                        ...
                        className="px-3 py-1.5 bg-purple-50 text-purple-600 border border-purple-200 rounded-md hover:bg-purple-100 disabled:opacity-50 flex items-center space-x-2 text-xs font-semibold"
                    >
                        {enhancingId === intern.id ? (<span>Enhancing...</span>) : <span>Enhance with AI</span>}
                    </button>
                    <button onClick={() => removeDynamicItem('internships', intern.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs font-semibold">Remove</button>
                </div>
            </div>
        ))}
        <button onClick={() => addDynamicItem('internships', {id: crypto.randomUUID(), title: '', date: '', description: ''})} className="mt-4 w-full py-2 border-2 border-dashed border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 hover:border-purple-300 transition-colors font-semibold text-sm">+ Add Experience</button>
       </FormSection>
       ...
       >
        {resumeData.projects.map((proj) => (
            <div key={proj.id} className="border-b border-gray-100 pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                ...
                <div className="flex items-center justify-between mt-3">
                    ...
                    <button onClick={() => removeDynamicItem('projects', proj.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs font-semibold">Remove</button>
                </div>
            </div>
        ))}
        <button onClick={() => addDynamicItem('projects', {id: crypto.randomUUID(), name: '', date: '', description: ''})} className="mt-4 w-full py-2 border-2 border-dashed border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 hover:border-purple-300 transition-colors font-semibold text-sm">+ Add Project</button>
       </FormSection>
        ...
        >
            <TextArea 
                ...
                name="skills" 
                value={resumeData.skills.map(s => s.skills).join(', ')} 
       ...
       <FormSection 
            title="Languages"
            icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m1.204 8.845a4 4 0 011.796 3.155H4a8 8 0 011.796-3.155M9 17v-4m0 0V9m0 4l-4-4m4 4l4-4" /></svg>}
        >
        {resumeData.languages.map((lang) => (
            <div key={lang.id} className="border-b border-gray-100 pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                <Input label="Language" name="language" value={lang.language} onChange={(e) => handleDynamicChange('languages', lang.id, e)} />
                <Input label="Proficiency" name="proficiency" value={lang.proficiency} onChange={(e) => handleDynamicChange('languages', lang.id, e)} />
                <div className="flex justify-end">
                    <button onClick={() => removeDynamicItem('languages', lang.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs font-semibold">Remove</button>
                </div>
            </div>
        ))}
        <button onClick={() => addDynamicItem('languages', {id: crypto.randomUUID(), language: '', proficiency: ''})} className="mt-4 w-full py-2 border-2 border-dashed border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 hover:border-purple-300 transition-colors font-semibold text-sm">+ Add Language</button>
       </FormSection>

       <FormSection 
            title="Achievements"
            icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>}
       >
        ...
        <button onClick={() => addDynamicItem('achievements', {id: crypto.randomUUID(), description: ''})} className="mt-4 w-full py-2 border-2 border-dashed border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 hover:border-purple-300 transition-colors font-semibold text-sm">+ Add Achievement</button>
       </FormSection>
         ...
         <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg max-w-2xl w-full">
              ...
              <div className="mt-4 flex justify-end space-x-2">
                ...
                <button onClick={handleCropImage} className="px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600">Crop & Save</button>
              </div>
            </div>
         </div>
       )}
